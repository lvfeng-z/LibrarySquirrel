...
// model
// 表单数据
const formData = defineModel<SiteDomain>('formData', { required: true })
// 弹窗开关
...
async function handleSaveButtonClicked() {
  if (props.submitEnabled) {
    if (props.mode === DialogMode.NEW) {
      ...
      const response = await apis.siteDomainSave(tempFormData)
    ...
    if (props.mode === DialogMode.EDIT) {
      const tempFormData = lodash.cloneDeep(formData.value)
      const response = await apis.siteDomainUpdateById(tempFormData)
      if (ApiUtil.check(response)) {
        emits('requestSuccess')
        state.value = false
      }
