...
export default class TaskDao extends BaseDao<TaskQueryDTO, Task> {
  ...
  async queryParentPage(page: Page<TaskQueryDTO, Task>) {
    ...
    const whereClause = isNotBlank(tempWhereClause) ? tempWhereClause : ''
    ...
    // 查询
    const rows = await Database.all<unknown[], Record<string, unknown>>(statement, modifiedPage.query)
  ...
  async setTaskTreeStatus(taskIds: number[], status: TaskStatusEnum, includeStatus?: TaskStatusEnum[]): Promise<number> {
    ...
    const statement = `WITH children AS (SELECT id, is_collection, IFNULL(pid, 0) AS pid, task_name, site_id, site_work_id, url, create_time, update_time, status,
                                                pending_resource_id, continuable, plugin_public_id, plugin_contribution_id, plugin_data, error_message FROM task WHERE id in (${idsStr}) AND is_collection = 0),
                            parent as (SELECT id, is_collection, 0 as pid, task_name, site_id, site_work_id, url, create_time, update_time, status, pending_resource_id,
                                              continuable, plugin_public_id, plugin_contribution_id, plugin_data, error_message FROM task WHERE id in (${idsStr}) AND is_collection = 1)
  ...
  async listTaskTree(taskIds: number[], includeStatus?: TaskStatusEnum[]): Promise<TaskTreeDTO[]> {
    const idsStr = taskIds.join(',')
    const statusStr = includeStatus?.join(',')
    const statement = `WITH children AS (SELECT id, is_collection, IFNULL(pid, 0) AS pid, task_name, site_id, site_work_id, url, create_time, update_time, status, pending_resource_id,
                                                continuable, plugin_public_id, plugin_contribution_id, plugin_data, error_message FROM task WHERE id in (${idsStr}) AND is_collection = 0),
                            parent as (SELECT id, is_collection, 0 as pid, task_name, site_id, site_work_id, url, create_time, update_time, status, pending_resource_id,
                                              continuable, plugin_public_id, plugin_contribution_id, plugin_data, error_message FROM task WHERE id in (${idsStr}) AND is_collection = 1)
                       ...
                       UNION
                       SELECT id, is_collection, 0 as pid, task_name, site_id, site_work_id, url, create_time, update_time, status, pending_resource_id,
                              continuable, plugin_public_id, plugin_contribution_id, plugin_data, error_message
                       FROM task
    ...
    const sourceTasks = super.toResultTypeDataList<TaskTreeDTO>(rows)
  ...
  async listStatus(ids: number[]): Promise<TaskScheduleDTO[]> {
    ...
    const statement = `SELECT id, pid, status, CASE WHEN status = ${TaskStatusEnum.FINISHED} THEN 100 END AS schedule
                       ...
                       WHERE id in (${idsStr})`
    const rows = await Database.all<unknown[], Record<string, unknown>>(statement)
    return super.toResultTypeDataList<TaskScheduleDTO>(rows)
  }
}
