...
export interface PluginContext {
  ...
  slots: {
    /** 注册嵌入插槽 (对应 EmbedSlot) */
    registerEmbedSlot: (config: Omit<EmbedSlotConfig, 'pluginId' | 'type'>) => Promise<void>
    /** 注册面板插槽 (对应 PanelSlot) */
    registerPanelSlot: (config: Omit<PanelSlotConfig, 'pluginId' | 'type'>) => Promise<void>
    /** 注册视图插槽 (对应 ViewSlot) */
    registerViewSlot: (config: Omit<ViewSlotConfig, 'pluginId' | 'type'>) => Promise<void>
    /** 注册菜单插槽 (对应 MenuSlot) */
    registerMenuSlot: (config: Omit<MenuSlotConfig, 'pluginId' | 'type'>) => Promise<void>
    /** 注册站点浏览器列表插槽 (对应 siteBrowserList) */
    registerSiteBrowserSlot: (config: Omit<SiteBrowserListSlotConfig, 'pluginId' | 'type'>) => Promise<void>
    /** 注销插槽 */
